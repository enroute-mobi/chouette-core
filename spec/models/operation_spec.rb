...
RSpec.describe Operation do
  class self::Test < Operation # rubocop:disable Lint/ConstantDefinitionInBlock,Style/ClassAndModuleChildren
    attr_accessor :id, :workgroup, :error_uuid, :status, :user_status, :started_at, :ended_at, :creator

    def persisted=(persisted)
      self.id = persisted ? Kernel.rand(10_000) : nil
    end
  ...
  describe '#perform' do
    context 'when perform experiences no error' do
      ...
      it 'changes user status from pending to successful' do
        expect do
          operation.perform
        end.to change(operation, :user_status).from(Operation.user_status.pending).to(Operation.user_status.successful)
      end
    end
    ...
    context 'when perform experiences an error' do
      ...
      it 'defines an error uuid' do
        expect do
          operation.perform
        end.to change(operation,
                      :error_uuid).from(nil).to(a_string_matching(/^[0-9a-f]{8}\b-[0-9a-f]{4}\b-[0-9a-f]{4}\b-[0-9a-f]{4}\b-[0-9a-f]{12}$/))
      end

      it 'changes user status from pending to failed' do
        expect do
          operation.perform
        end.to change(operation, :user_status).from(Operation.user_status.pending).to(Operation.user_status.failed)
      end
    end
...
RSpec.describe Operation::Callback::Invoker do
  class self::Test < Operation::Callback # rubocop:disable Lint/ConstantDefinitionInBlock,Style/ClassAndModuleChildren
