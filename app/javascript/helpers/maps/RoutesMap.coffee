...
export default class RoutesMap extends Map
  ...
  addRoute: (route)->
    ...
    stops.forEach (stop, i) =>
      if stop.longitude && stop.latitude
        ...
        unless @seenStopIds.includes(stop.stop_area_id)
          @area.push [parseFloat(stop.longitude), parseFloat(stop.latitude)]
          @seenStopIds.push stop.stop_area_id
  ...
  addLabels: (resourceName) ->
    menu = new LayersControl(@routes, this)
    @map.addControl(menu)
    @map.addControl new LayersButton(menu: menu)
