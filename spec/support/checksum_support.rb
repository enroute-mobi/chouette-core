...
shared_examples 'checksummed model' do
  let( :stripped_base_atts ){ strip_values( base_atts )  }
  let( :stripped_same_atts ){ strip_values( same_checksum_atts ) }
  ...
  it 'creates checksums depending on certain attribute values' do
    ...
    modify_atts(base_atts).each do | delta_atts |
      expect( create(factory, **strip_values(delta_atts)).checksum ).not_to eq(reference.checksum)
    end
  end
end
