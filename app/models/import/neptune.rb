class Import::Neptune < Import::Base
  ...
  def import_without_status
    ActiveRecord::Base.cache do
      ApplicationModel.skipping_objectid_uniqueness do
        ...
        import_resources :stop_areas, :lines_content, :missing_checksums
      end
    end
  end
  ...
  def import_time_tables
    Chouette::TimeTable.skipping_objectid_uniqueness do
      ...
      each_element_matching_css('ChouettePTNetwork Timetable') do|source_timetable, filename, progress|
        tt = Chouette::TimeTable.includes(:dates, :periods).find_or_initialize_by objectid: source_timetable[:object_id]
  ...
  def import_stop_areas
    ...
    stop_area_ids = []
    each_element_matching_css('ChouettePTNetwork ChouetteArea StopArea') { |s| stop_area_ids << s[:object_id] }
    ...
    Chouette::StopArea.within_workgroup(workgroup) do
      each_element_matching_css('ChouettePTNetwork ChouetteArea') do |source_parent, filename, progress|
        ...
        each_element_matching_css('StopArea', source_parent) do |source_stop_area|
          stop_area = existing_stop_areas[source_stop_area[:object_id]] || stop_area_referential.stop_areas.light.build(registration_number: source_stop_area[:object_id])
  ...
  def import_vehicle_journeys_in_line(line, source_vehicle_journeys)
    profile_tag :import_vehicle_journeys_in_line do
      ...
      source_vehicle_journeys.each do |source_vehicle_journey|
        ...
        TimeTablesVehicleJourney.bulk_insert do |w|
          (@time_tables.delete(source_vehicle_journey[:object_id]) || []).each do |time_table_id|
            w.add vehicle_journey_id: vehicle_journey.id, time_table_id: time_table_id
          end
        end
      end
    end
  ...
  def add_stop_points_to_route(route, link_ids, links, route_object_id)
    ...
    Chouette::StopPoint.bulk_insert do |worker|
      link_ids.each_with_index do |link_id, i|
        ...
        add_stop_point_to_route(stop_point_id, route, i, route_object_id, worker)
        ids << stop_point_id
      end
      ids << last_point_id
      add_stop_point_to_route(last_point_id, route, route.stop_points.size, route_object_id, worker)
    end
    @stop_points[route_object_id] = route.stop_points.reload.light.includes(:stop_area_light).each_with_index.map{|s, i| [ids[i], s]}.to_h
  end

  def add_stop_point_to_route(stop_point_id, route, pos, route_object_id, worker)
    stop_area_id = @parent_stop_areas[stop_point_id]
    stop_point = Chouette::StopPoint.new stop_area_id: stop_area_id, position: pos
    ...
    worker.add stop_area_id: stop_area_id, position: pos, route_id: route.id, objectid: @objectid_formatter.objectid(stop_point).to_s
  end
  ...
  def add_stop_points_to_journey_pattern(journey_pattern, stop_point_ids, route_object_id)
    ...
    JourneyPatternsStopPoint.bulk_insert do |w|
      stop_point_ids.each do |stop_point_id|
        w.add(stop_point_id: @stop_points[route_object_id][stop_point_id].id, journey_pattern_id: journey_pattern.id)
      end
    end
    ...
    Chouette::VehicleJourneyAtStop.bulk_insert do |worker|
      vehicle_journey_at_stops.sort_by{|i| i[:order]}.each do |source_vehicle_journey_at_stop|
        vjas = Chouette::VehicleJourneyAtStop.new(
          stop_point: @stop_points[route_object_id][source_vehicle_journey_at_stop[:stop_point_id]],
          arrival_local_time: source_vehicle_journey_at_stop[:arrival_time],
          departure_local_time: source_vehicle_journey_at_stop[:departure_time],
          vehicle_journey_id: vehicle_journey.id,
        )
        worker.add(vjas.attributes)
      end
    end
  end
