...
export default function StopPointsMap({ index, onSelectMarker, onUpdateViaOlMap, onUnselectMarker, value }) {
  ...
  const onMapInit = async map => {
    const fetchedFeatures = await (await fetch(`${path.build({ workbenchId, referentialId, lineId, routeId })}/retrieve_nearby_stop_areas?stop_area_id=${stoparea_id}&target_type=zdep`)).json()
