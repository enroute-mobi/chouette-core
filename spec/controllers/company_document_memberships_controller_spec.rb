...
RSpec.describe CompanyDocumentMembershipsController, type: :controller do
  ...
  describe 'GET #index' do
    ...
    let(:user_can_update_companies) { true }
    let(:request) { get :index, params: { workbench_id: workbench.id, company_id: company.id } }
    ...
    before do
      ...
      allow(fk_policy).to receive(:update?).and_return(user_can_update_companies)
    ...
    context 'when user cannot update companies' do
      let(:user_can_update_companies) { false }
