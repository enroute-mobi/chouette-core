  ...
  describe 'action links for' do
    context 'unarchived referential' do
      context 'no rights' do
        it 'has only show, Calendar and Service Count actions' do
      ...
      context 'all rights and same organisation' do
          ...
          it 'has corresponding actions' do
            ...
            expect_action_link_hrefs(action).to match_array(
              [
                ...
                archive_workbench_referential_path(current_workbench, object),
                [current_workbench, object]
              ]
            )
          end
        ...
        context 'on show' do
          let(:action) { :show }
          it 'has corresponding actions' do # rubocop:disable Metrics/BlockLength
            expect_action_link_elements(action).to match_array(
              [
                'Courses',
                'Editer ce jeu de données',
            ...
            expect_action_link_hrefs(action).to match_array(
              [
                ...
                new_workbench_control_list_run_path(current_workbench, referential_id: object.id),
                archive_workbench_referential_path(current_workbench, object),
                new_workbench_referential_clean_up_path(current_workbench, object),
                [current_workbench, object]
              ]
            )
          end
