module Chouette
  class TimeTable < Chouette::TridentActiveRecord
    ...
    def self.scheduled_on(date)
      ...
      query = "(int_day_types >> :day_order & 1 = 1 AND :date between period_start and period_end AND NOT (date = :date AND in_out = false)) OR (date = :date AND in_out = true)"
      joins(:periods, :dates).where(query, date: date, day_order: day_order)
    end
