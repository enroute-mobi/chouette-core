...
RSpec.describe ReferentialInserter do
  ...
  subject do
    ReferentialInserter.new context.referential(:target)  do |config|
  ...
  def dummy_copy_vehicle_journeys
    ...
    target_route_id = target.switch{Chouette::VehicleJourney.first.route_id}

    source_journey_pattern_id = Chouette::VehicleJourney.first.journey_pattern_id
    target_journey_pattern_id = target.switch{Chouette::VehicleJourney.first.journey_pattern_id}
  ...
  describe "Vehicle Journey" do
    before do
      ...
      @vj_target_init_count = target.switch{Chouette::VehicleJourney.count}
      @vj_target_max_id = target.switch{Chouette::VehicleJourney.maximum(:id)}
    ...
    it "creates the right number of new records in the target referential" do
      expect(target.switch{Chouette::VehicleJourney.count}).to eq(@vj_source_init_count+@vj_target_init_count)
    end

    it "creates new records in the target referential with the right id" do
      expect(target.switch{Chouette::VehicleJourney.maximum(:id)}).to eq(@vj_target_max_id+@vj_source_init_count)
    end
  ...
  describe "Vehicle Journey At Stops" do
    before do
      ...
      @vjas_target_init_count = target.switch{Chouette::VehicleJourneyAtStop.count}
      @vjas_target_max_id = target.switch{Chouette::VehicleJourneyAtStop.maximum(:id)}
    ...
    it "creates the right number of new records in the target referential" do
      expect(target.switch{Chouette::VehicleJourneyAtStop.count}).to eq(@vjas_source_init_count+@vjas_target_init_count)
    end

    it "creates new records in the target referential with the right id" do
      expect(target.switch{Chouette::VehicleJourneyAtStop.maximum(:id)}).to eq(@vjas_target_max_id+@vjas_source_init_count)
    end
  end
  ...
  describe "TimeTablesVehicleJourney" do
    before do
      ...
      @ttvj_target_init_count = target.switch{Chouette::TimeTablesVehicleJourney.count}
      ...
      @target_time_table_1_id = target.switch{ Chouette::TimeTable.first.id }
      @target_time_table_2_id = target.switch{ Chouette::TimeTable.last.id }
