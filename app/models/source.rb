...
class Source < ApplicationModel
  ...
  def import_option_tag
    workbench.tags.where(id: import_option_tag_id)
  end

  def import_option_tag=(value)
    import_options["import_option_tag_id"] = value.reject(&:blank?)
  end
  ...
  class Retrieval < Operation
    ...
    def import_attributes
      import_options = import_workbench_options
      {
        ...
        creator: creator,
        file: imported_file,
        options: import_options.except('code_space_id', 'tag_id'),
        type: 'Import::Workbench',
        import_category: import_category,
        code_space_id: import_options['code_space_id']
      }.tap do |import_params|
        if (tags = import_options['tag_id']).is_a?(Array)
