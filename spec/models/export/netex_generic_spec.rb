...
RSpec.describe Export::NetexGeneric do
  ...
  describe '#netex_profile' do
    ...
    context 'when profile "idm/full" is selected (legacy value)' do
      let(:export) { Export::NetexGeneric.new(setup: { profile: 'idfm/full' }) }
  ...
  describe Export::NetexGeneric::AlternateIdentifiersExtractor::Decorator do
    ...
    describe '#registration_number_value' do
      ...
      context 'when registration_number is "dummy"' do
        ...
        it { is_expected.to eq([%w[external dummy]]) }
      end
    ...
    describe '#alternate_identifiers_values' do
      ...
      context "when registration_number_value is [['external', 'dummy']]" do
        before do
          allow(decorator).to receive(:registration_number_value) { [%w[external dummy]] }
        end

        context "when codes_values is [['first', '1']]" do
          before do
            allow(decorator).to receive(:codes_values) { [%w[first 1]] }
          end

          it { is_expected.to eq([%w[external dummy], %w[first 1]]) }
        end
      end
    end
  ...
  describe 'Lines export' do
    describe Export::NetexGeneric::Lines::Decorator do
      ...
      describe '#accessibility' do
        ...
        let(:context) do
          Chouette.create do
            line :line, wheelchair_accessibility: 'yes', step_free_accessibility: 'no',
      ...
      describe '#key_list' do
        ...
        context 'when company has a registration number' do
          ...
          it 'generate key_list' do
            is_expected.to include(Netex::KeyValue.new(key: 'external', value: 'RN',
  ...
  describe 'Routes export' do
    ...
    it 'create a Netex::Route for each Chouette Route and a Netex::Direction for routes having a published_name' do
      ...
      routes_resources.each do |resource|
        route = Chouette::Route.find_by! objectid: resource.id
        ...
        next unless route.published_name
    ...
    describe Export::NetexGeneric::Routes::Decorator::LineRoutingConstraintZoneDecorator do
      ...
      context 'when two routes have the same stop_areas for each stop_point' do
        it 'create a Netex::Route for each Chouette Route with a Netex::RoutingConstraintZone' do
          expect(routing_constraint_zone_resources.map(&:name)).to match_array([line_routing_constraint_zone.name,
                                                                                line_routing_constraint_zone.name])
  ...
  describe 'RoutingConstraintZones export' do
    ...
    describe Export::NetexGeneric::RoutingConstraintZones::Decorator do
      ...
      let(:decorator) do
        Export::NetexGeneric::RoutingConstraintZones::Decorator.new routing_constraint_zone,
    ...
    describe Export::NetexGeneric::StopAreas::Decorator do
      ...
      describe '#netex_attributes' do
        ...
        it 'uses StopArea transport mode as netex transport mode and sub mode' do
          ...
          is_expected.to include(transport_mode: a_string_eq_to('bus'),
                                 transport_submode: a_string_eq_to('regionalBus'))
        end
      end
      ...
      describe '#netex_resource' do
        ...
        context "when Stoparea transport mode is 'bus/regional_bus'" do
          before do
            allow(decorator).to receive(:transport_mode).and_return(Chouette::TransportMode.from('bus/regional_bus'))
          end
  ...
  describe 'JourneyPatterns export' do
    ...
    it 'create a Netex::JourneyPattern for each Chouette JourneyPattern' do
      ...
      jp_resources.each do |resource|
        jp = Chouette::JourneyPattern.find_by! objectid: resource.id

        expect(jp).to be

        next unless jp.published_name

        expect(resource.destination_display_ref).to be
        expect(resource.destination_display_ref.ref).to eq(jp.objectid.gsub(/j|JourneyPattern/) do
                                                             'DestinationDisplay'
                                                           end)
    ...
    describe Export::NetexGeneric::TimeTables::Decorator do
      ...
      let(:context) do
        Chouette.create do
          ...
          time_table objectid: 'objectid::LOC',
                     codes: { many_codes: %w[first_code second_code], one_code: %w[uniq_code] }
        end
      end
      ...
      describe '#exported_periods' do
        ...
        context 'when a validity period exists' do
          ...
          let(:context) do
            Chouette.create do
              time_table periods: ['2023-02-01'.to_date..'2023-02-15'.to_date], int_day_types: 272,
          ...
          let(:decorated_time_table) do
            Export::NetexGeneric::TimeTables::Decorator.new(time_table, validity_period: validity_period)
          end
          ...
          context 'when an period intersects the validity period' do
            subject do
              decorated_time_table.exported_periods.find do |ep|
                ep&.from_date == '2023-02-01'.to_date && ep&.to_date == '2023-02-15'.to_date
              end.present?
            end
